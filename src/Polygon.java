...
public class Polygon implements Rotatable {
	...
	public Polygon(Direction d, int sides, int radius, double interval){
		currDirection = d;
		this.sides = sides;
		this.radius = radius;
		this.theta = 0;
	...
	public Polygon(Direction d, int sides, int radius, double interval, double theta){
		currDirection = d;
		this.sides = sides;
		this.radius = radius;
		this.theta = theta;
	...
	public ArrayList<CartesianPoint> currentState() {
		...
		for(int i = 1; i < sides+1; i++){
			curr.add(new PolarPoint(radius, theta + (Math.PI * 2)/i).toCartesian());
		}
		return curr;
	}
	@Override
